...
| `users`               | Identity; linked to Keycloak `auth_id`; interop; `whatsapp` (nullable) is the outbound WhatsApp-notification recipient number — no self-service way to set it yet, seed/DB-set only (API_SPEC §11.14)                             |
| `members`             | Org membership + role (RBAC)                                                                                                                                                                                                      |
| `customers`           | Customers per org; channel identity columns (`whatsapp`, `email`) are the connector match keys for inbound webhooks                                                                                                               |
| `products`            | SKUs, prices, cost, reorder point                                                                                                                                                                                                 |
| `inventory_movements` | stock ledger (in/out); on-hand = `sum(IN) − sum(OUT) + sum(ADJUST)` per product (convention used by task planning, §5)                                                                                                            |
